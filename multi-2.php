...
foreach ($chunks as $lote) {
    ...
    foreach ($multiCurl as $id => $curl) {
        ...
        // Asegurarse de que las variables se actualizan por cada tracker
        ...
        $imei = $item->source->device_id;  // Aquí obtenemos el imei correctamente de cada tracker
        $group = $item->group_id;

        // Consultar la dirección de cada tracker
        $curlGeo = curl_init();
        curl_setopt_array($curlGeo, array(
        ...
        $responseGeo = curl_exec($curlGeo);
        curl_close($curlGeo);

        $json1 = json_decode($responseGeo);
